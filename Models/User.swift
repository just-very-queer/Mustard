...
struct User: Identifiable, Codable, Hashable {
 ...
 func encode(to encoder: Encoder) throws {
  ...
  if let created_at = created_at{ //Only encode if not nil - Keep as is
...
extension User {
 init(from account: Account) {
  ...
  self.locked = account.isLocked // Use the optional property directly
  self.bot = account.isBot // Use the optional property directly
  self.discoverable = account.discoverable
  self.indexable = account.indexable
  self.group = false // Assuming it's not a group user by default - Keep default assumption if valid
  self.created_at = nil // or Date() if you want to default to current date, adjust as needed - Set to nil as created_at from Account might not be relevant/available, adjust if needed
  self.note = account.note
  self.url = account.url?.absoluteString //Handle optional URL and convert to String
  self.avatar = account.avatar?.absoluteString //Handle optional URL and convert to String
  self.avatar_static = account.avatar?.absoluteString // Update based on actual data - Use same avatar for static for now, adjust if different static_avatar is available in Account
  self.header = account.header?.absoluteString // Handle optional URL
  self.header_static = account.header_static?.absoluteString // Handle optional URL
  self.followers_count = account.followers_count // Use the optional property directly
  self.following_count = account.following_count // Use the optional property directly
  self.statuses_count = account.statuses_count // Use the optional property directly
  self.last_status_at = account.last_status_at
  self.suspended = account.suspended // Use the optional property directly
  self.hide_collections = false // Default to false or based on your logic - Keep default assumption if valid
  self.noindex = false // Default to false - Keep default assumption if valid
  self.source = nil // Add if needed, based on your logic - Keep nil for now, add mapping if needed from Account to User source
  self.emojis = [] // Add emojis if needed - Keep empty array for now, add mapping if relevant from Account
  self.roles = [] // Add roles if needed - Keep empty array for now, add mapping if relevant from Account
  self.fields = [] // Add fields if needed - Keep empty array for now, add mapping if relevant from Account
 }
}
