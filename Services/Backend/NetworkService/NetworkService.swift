...
////  REVISED: Updated postAction to attempt decoding Post response.
////
//
//import Foundation
//import OSLog
...
//    }
//
//    /// Performs a POST action (e.g., liking or reblogging a post) and attempts to decode the updated Post response.
//    ///
//    /// - Parameters:
//    ///   - postID: The ID of the post to perform the action on.
//    ///   - path: The API path for the action (e.g., "/api/v1/statuses/{id}/favourite").
//    ///   - baseURLOverride: Optional URL to override the base URL from Keychain.
//    /// - Returns: The updated `Post` object if the API returns it, otherwise nil.
//    /// - Throws: `AppError` if a network error occurs.
//    func postAction(for postID: String, path: String, baseURLOverride: URL? = nil) async throws -> Post? {
//        // Ensure rate limiting
//        guard await rateLimiter.tryConsume() else {
//            throw AppError(type: .mastodon(.rateLimitExceeded))
//        }
...
//
//        // Perform the network request and attempt to decode the Post response
//        // If the API returns just 200 OK with no body, or a different body, decoding will fail,
//        // which is handled gracefully (returns nil) by performRequestOptional.
//        return try await performRequestOptional(request: request, responseType: Post.self)
//    }
//
...
//    }
//
//    /// Performs the actual network request, logs details, validates the response, and optionally decodes it.
//    /// Returns nil if decoding fails or the response is empty, instead of throwing a decoding error.
...
//            try validateResponse(response, data: data)
//
//            // Handle empty data case - valid 2xx but no body to decode
//            guard !data.isEmpty else {
//                 logger.debug("Received empty response body for optional decode, returning nil.")
//                 return nil
//             }
//
//            // Attempt to decode, return nil on failure instead of throwing
//            return try? jsonDecoder.decode(T.self, from: data)
//
//        } catch let urlError as URLError {
//            logger.error("Network request failed with URLError: \(urlError.localizedDescription)")
...
//            }
//        } catch let appError as AppError {
//            // Rethrow validation errors or other AppErrors
//            throw appError
//        } catch {
//            // Catch other unexpected errors during session.data(for:)
//            logger.error("Network request failed with unknown error: \(error.localizedDescription)")
//            throw AppError(type: .other("Unknown network error"), underlyingError: error)
//        }
//    }
...
//            // Map specific HTTP error codes to AppError types if needed
...
//            // Add other specific mappings (e.g., 400, 429)
...
//        // Ensure the request is built correctly within fetchData or manually here
//         let req = try buildRequest(url: userFetchURL, method: "GET", accessToken: accessToken)
//         let user: User = try await performRequest(request: req, responseType: User.self)
//
//        // let user: User = try await fetchData(url: userFetchURL, method: "GET", type: User.self) // Simplified call assumes fetchData handles token internally
//        logger.info("Successfully fetched current user: \(user.username, privacy: .public)")
//        return user
//    }
//
//     // --- ADDED: Fetch Post Context ---
//     /// Fetches the context of a post (ancestors and descendants/replies).
//     ///
//     /// - Parameter postId: The ID of the post to fetch context for.
//     /// - Returns: A `PostContext` object containing ancestors and descendants.
//     /// - Throws: `AppError` if the request fails.
//     func fetchPostContext(postId: String) async throws -> PostContext {
//         let endpoint = "/api/v1/statuses/\(postId)/context"
//         logger.debug("Fetching context for post ID: \(postId)")
//
...
//
//         // Build Request
//         let request = try buildRequest(url: url, method: "GET", accessToken: accessToken)
//
//         // Perform Request and Decode
//         return try await performRequest(request: request, responseType: PostContext.self)
//     }
//
...
//        if let responseBody = String(data: data, encoding: .utf8) {
//            // Limit log size for large responses if necessary
...
//        let queryItems = [URLQueryItem(name: "q", value: query), URLQueryItem(name: "type", value: "statuses")] // Specify type=statuses
//        let url = try await endpointURL(endpoint, queryItems: queryItems)
//
//        // The search endpoint (/api/v2/search) returns a SearchResults object, not [Post]
//        // We need to decode SearchResults and extract the statuses.
//        let searchResult: SearchResults = try await fetchData(url: url, method: "GET", type: SearchResults.self)
...
//        let endpoint = "/api/v1/trends/statuses" // Correct endpoint for trending statuses
//        let url = try await endpointURL(endpoint)
//
...
//}
//
//// --- ADDED: PostContext Struct ---
///// Represents the context of a status, including ancestors and descendants (replies).
