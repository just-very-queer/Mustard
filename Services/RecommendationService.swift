...
class RecommendationService: ObservableObject {
    ...
    private func getContext() throws -> ModelContext {
        guard let context = modelContext else {
            ...
            throw AppError.custom(message: errorMsg) // Define AppError.custom or use a specific error
        }
        return context
    }
    ...
    func scoredTimeline(_ timeline: [Post]) async -> [Post] {
        guard let currentContext = try? getContext() else { return timeline } // Return original if context fails
    ...
    func getInterestScore(for postID: String, authorAccountID: String?, tags: [String]?) async -> Double {
        ...
        // let fetchContext = self.modelContext // Use instance context // No, use currentContext
        ...
        if let authorAccountID = authorAccountID, !authorAccountID.isEmpty {
            ...
            do {
                if let userAffinity = try currentContext.fetch(userAffinityDescriptor).first {
                    ...
                    // logger.debug("PostID \(postID): Author \(authorAccountID) affinity score: \(userAffinity.score)")
                }
            } catch {
        ...
        if let postTags = tags, !postTags.isEmpty {
            // Fetching relevant tags one by one to simplify predicate logic with array.contains
            // This can be optimized if performance becomes an issue for posts with many tags.
            ...
            for tagName in postTags {
                ...
                do {
                    if let hashtagAffinity = try currentContext.fetch(hashtagAffinityDescriptor).first {
                        ...
                        // logger.debug("PostID \(postID): Tag \(tagName) affinity score: \(hashtagAffinity.score)")
                    }
                } catch {
        ...
        // logger.info("Calculated interest score for post \(postID): \(score)")
        return score
    }
}
